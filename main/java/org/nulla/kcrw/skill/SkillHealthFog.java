...
public class SkillHealthFog extends Skill {
	...
	public boolean onUse(final EntityPlayer player) {
		...
		List list = player.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.fromBounds(
		...
		for(int i = 0; i < 32; i++) {
			...
			Vec3 vec = new Vec3(8 * ran.nextFloat() - 4, 3 * ran.nextFloat() - 1 - player.eyeHeight, 8 * ran.nextFloat() - 4).normalize();
