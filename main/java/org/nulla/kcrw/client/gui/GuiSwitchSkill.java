	...
	public void initGui() {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (skillsInSlot[i] != null) {
				buttonList.add(btnCurrentSkillSlot[i] = new GuiButtonImage(i, (int) (width * (0.35 + 0.1 * i) - 16),
						(int) (height * 0.3), 32, 32, skillsInSlot[i].mIcon, true));
			} else {
				buttonList.add(btnCurrentSkillSlot[i] = new GuiButtonImage(i, (int) (width * (0.35 + 0.1 * i) - 16),
						(int) (height * 0.3), 32, 32, Skill.empty_skill_icon, true));
			}
		}
